 ...
 *  Contributer: [NeuroBinds, ArchEmits]
 *  
 *  Not For Sale.
 * 
...
/*Plugins OnEnable*/
   public function onEnable(){	
		if(!is_dir($this->getPluginDir())){
			@mkdir($this->getServer()->getDataPath()."plugins/ArchCoreSystem/ArchPlayBox");
		}

		if(!$this->CheckMusic()){
			$this->getLogger()->info("\n§cError There No Songs.\n§bPlease Put .nbs file at ArchPlayBox Folder!");
		}else{
			$this->StartNewTask();
		}
...
//load level/world
        $this->getServer()->loadLevel("ARCHRPG_LOBBY_BEGINNER");
/*Server Name*/
...
/*Plugin AddParticle*/
   public function addParticle(PlayerJoinEvent $event){ 
       $level = $this->getServer()->getLevelByName("ARCHRPG_LOBBY_BEGINNER");
...
/*Plugin OnBreakStats*/
	...
	public function onSongCommands(CommandSender $sender, Command $cmd, $label, array $args) {
		switch($cmd->getName()) {
			case "song":
				if(isset($args[0])){
					switch($args[0]){
	...
	public function getPluginDir(){
		return $this->getServer()->getDataPath()."plugins/ArchCoreSystem/ArchPlayBox/";
	}
